...
/* Plugin Name: WP Force Password
 * Description: WP Force Password is a plugin that forces the users to change their password periodically and keep your website secure from hack attack and password theft.
 * Plugin URI: https://wordpress.org/plugins/wp-force-password
...
if ( ! function_exists( 'wpfp_plugin_activate' ) ) {
    function wpfp_plugin_activate() {
        ...
        if ( 'production' !== wp_get_environment_type() ) {
            update_option('wpfp_shut_off_email', 1);
        }   
    }
}
...
/**
 * Update disable notification true option on plugin load
 *
 * @return void
 */
function wpfp_plugin_loaded_action() {
    if(get_option('wpfp_shut_off_email') != 1) { 
        if ( 'production' !== wp_get_environment_type() ) { 
             update_option('wpfp_shut_off_email', 1);
...
add_action( 'plugins_loaded', 'wpfp_plugin_loaded_action' );
